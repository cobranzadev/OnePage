...
#st.markdown('<div class="filtro-label">Coordinador: <span style="color:#9DB8FF;">Todas</span></div>', unsafe_allow_html=True)
coordinadores = sorted(df['nombre'].unique())
coordinador_sel = st.selectbox("Selecciona un coordinador", ["Todos"] + coordinadores)

if coordinador_sel != "Todos":
    df_filtrado = df[df["nombre"]==coordinador_sel]
else:
    df_filtrado = df.copy()
